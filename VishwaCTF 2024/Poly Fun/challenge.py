...
# print(enc)
file = open("CTF_Challenge/VishwaCTF 2024/Poly Fun/encoded_key.txt", "rb")
...
for i in range(256):
    try:
        lib[chr(poly(i)).encode()] = i
    except:
        continue

key = b""
while len(encoded_key) > 0:
    check = False
    for i in range(1, len(encoded_key) + 1):
        add = encoded_key[:i]

        if add in lib:
            key += int.to_bytes(lib[add])
            check = True
            encoded_key = encoded_key[i:]
            break
    print(key)
    if check == False:
        break
...
file = open("CTF_Challenge/VishwaCTF 2024/Poly Fun/encoded_flag.txt", "rb")
encoded_flag = file.read()

for i in range(1, 13):
    try:
        cipher = AES.new(key, mode=i)
        print(cipher.decrypt(b64decode(encoded_flag)))
    except:
        continue
