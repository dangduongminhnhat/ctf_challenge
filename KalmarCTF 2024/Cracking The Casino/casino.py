...
r.recvline()
r.recvline()
q = int(r.recvline().decode()[:-1].split("q = ")[1], 10)
g = int(r.recvline().decode()[:-1].split("g = ")[1], 10)
h = int(r.recvline().decode()[:-1].split("h = ")[1], 10)
print(r.recvuntil(b"[C]ards"))
r.sendline(b"o")

for _ in tqdm(range(624)):
    r.recvuntil(b"Are you able to guess the value? [Y]es/[N]o")
    r.sendline(b"N")

    rc.submit(int(r.recvline().decode()[:-1].split("commited value was ")[1], 10))
    r.recvline()
    r.recvline()
...
for i in range(1337):
    ...
    r.recvuntil(b'whats your guess?')
    r.sendline(str(rc.predict_randint(0, 2 ** 32 - 2)).encode())
    r.recvline()
    correct += 1
