...
print(r.recvuntil(b"you have one new encrypted message:\n"))
data = r.recvline().decode()[:-1]
data = bytes.fromhex(data)


def find_encrypted_message(message):
    r.recvuntil(b" > ")
    r.sendline(message.hex().encode())

    enc = bytes.fromhex(r.recvline().decode()[:-1])
    return enc[-16:]
...
for i in range(0, len(ciphertext), 16):
    enc_f = find_encrypted_message(flag[-16:])
    flag += strxor(ciphertext[i: i + 16], enc_f)
print(flag)
