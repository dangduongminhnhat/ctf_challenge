...
MASTER_KEY = b'\xc7\xb0\xa6k3\n?83\xd1h\xbbl\x15\x90\xa3'
...
# main()
msg = b'password reset: '
aim = b'doubledelete' + b"\x00" * 4
#
from pwn import *

r = remote("blocked1.wolvctf.io", 1337)

print(r.recvuntil(b"[      password reset portal      ]\n"))
username = r.recvline()[:-1].split(b"you are logged in as: ")[1] + b"\x00" * 4
print(username)
r.recvline()

print(r.recvuntil(b" > "))
r.sendline(b"2")
data = bytes.fromhex(r.recvline().decode()[:-1])
iv = data[:16]
ciphertext = data[16:]
new_iv = b""
for i in range(16):
    new_iv += int.to_bytes(ciphertext[i] ^ username[i] ^ aim[i])
send = (iv + new_iv + ciphertext[16:]).hex()
