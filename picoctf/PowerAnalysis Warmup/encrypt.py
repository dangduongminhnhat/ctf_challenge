#!/usr/bin/env python3
import random, sys, time
from pwn import *
import binascii

# with open("key.txt", "r") as f:
#     SECRET_KEY = bytes.fromhex(f.read().strip())

Sbox = (
    ,
)

# Leaks one bit of information every operation
leak_buf = []
def leaky_aes_secret(data_byte, key_byte):
    out = Sbox[data_byte ^ key_byte]
    leak_buf.append(out & 0x01)
    return out

# Simplified version of AES with only a single encryption stage
def encrypt(plaintext, key):
    global leak_buf
    leak_buf = []
    ciphertext = [leaky_aes_secret(plaintext[i], key[i]) for i in range(16)]
    return ciphertext

# Leak the number of 1 bits in the lowest bit of every SBox output
def encrypt_and_leak(plaintext):
    ciphertext = encrypt(plaintext, SECRET_KEY)
    ciphertext = None # throw away result
    time.sleep(0.01)
    return leak_buf.count(1)

# pt = input("Please provide 16 bytes of plaintext encoded as hex: ")
# if len(pt) != 32:
#     print("Invalid length")
#     sys.exit(0)
#
# pt = bytes.fromhex(pt)
# print("leakage result:", encrypt_and_leak(pt))


lib = {}
for m in range(8):
    n = 2 ** m
    arr = []
    for i in range(256 // n):
        count = 0
        for j in range(n):
            if Sbox[i * n + j] % 2 == 1:
                count += 1
        arr.append(count)
    lib[n] = arr
print(Sbox[0], Sbox[129], Sbox[131], Sbox[136], Sbox[138])
r = remote("saturn.picoctf.net", 63430)
r.recvuntil(b"Please provide 16 bytes of plaintext encoded as hex: ")
r.sendline(binascii.hexlify(int.to_bytes(138)) + b"00" * 15)
num1 = int(r.recvline().decode().split("leakage result: ")[1])
print(num1)

# keys = []
# for k in range(16):
#     save2 = {1: {}, 2: {}, 4: {}, 8: {}, 16: {}, 32: {}}
#     save = {}
#     for i in range(64):
#         send = b"00" * k + binascii.hexlify(int.to_bytes(i)) + b"00" * (15 - k)
#         r = remote("saturn.picoctf.net", 56282)
#         r.recvuntil(b"Please provide 16 bytes of plaintext encoded as hex: ")
#         r.sendline(send)
#         num = int(r.recvline().decode().split("leakage result: ")[1])
#         if num in save:
#             save[num] += 1
#         else:
#             save[num] = 1
#         if i + 1 in save2:
#             for n in save:
#                 save2[i + 1][n] = save[n]
#         r.close()
#     save2[64] = save
#     keys.append(save2)
#     # print(lib)
#     # print(save)
#     # print(save2)
# # k1 = {1: {8: 1, 9: 1}, 2: {8: 2, 9: 1}, 4: {8: 2, 9: 3}, 8: {8: 3, 9: 6}, 16: {8: 6, 9: 11}, 32: {8: 19, 9: 14}, 64: {8: 38, 9: 26}}
# # k2 = {1: {8: 1, 7: 1}, 2: {8: 2, 7: 1}, 4: {8: 3, 7: 2}, 8: {8: 5, 7: 4}, 16: {8: 10, 7: 7}, 32: {8: 14, 7: 19}, 64: {8: 26, 7: 38}}
# print(keys)
# print(lib)
