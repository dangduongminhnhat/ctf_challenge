...
print(r.recvline())
c = int(r.recvline().decode().split("Ciphertext of Flag: ")[1], 10)
r.sendline(b"KEY:")
N = int(r.recvline().decode().split("N:")[1], 10)
E = int(r.recvline().decode().split("E:")[1], 10)

c_inv = inverse(c, N)
r.sendline(b"DECRYPT:" + str(c_inv).encode())
print(r.recvline())
m_inv = int(r.recvline().decode(), 10)
print(long_to_bytes(inverse(m_inv, N)))
